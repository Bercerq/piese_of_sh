...
const PublisherReportModal = (props: ReportModalProps) => {
  const id = _.get(props, "report.id", -1);
  const owner = _.get(props, "report.scope.owner", "");
  const reportScope = "publisher"
  ...
  const [emailToOptions, setEmailToOptions] = useState<SelectOption[]>([]);
  ...
  async function loadTemplateFields() {
    const templates: ReportTemplate[] = await Api.Get({ path: `/api/publisherReportTemplates/${props.scopeId}` });
  ...
  function save(needsRun: boolean) {
    ...
    if (nameValidation.error || periodValidation.error || rangeValidation.error || scheduleValidation.error) {
      ...
      setSaving(true);
      const emailTo = emailToOptions.map((o) => { return o.value as string; });
...
export default PublisherReportModal;