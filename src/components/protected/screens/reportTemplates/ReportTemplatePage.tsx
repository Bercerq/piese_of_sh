...
const ReportTemplatePage = (props: ReportTemplatePageProps) => {
  ...
  const cancelClick = () => {
    history.push(`/report-templates/${props.scope}/${props.scopeId}`);
  }
  ...
  const handleTemplateSubmit = async (id: number, template: Partial<ReportTemplate>) => {
    ...
    try {
      if (id > 0) {
        ...
        notificationSystem.current.addNotification(NotificationOptions.success(<span>Template <strong>{template.name}</strong> saved.</span>, false, () => { history.push(`/reports/${props.scope}/${props.scopeId}/templates/${res.id}`); }));
      }
    } catch (err) {
