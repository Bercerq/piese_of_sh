...
const ReportTemplatesTable = (props: ReportTemplatesTableProps) => {
  ...
  const editFormatter = (cellContent, row) => {
    if (row.writeAccess) {
      return <Link to={`/report-templates/${props.scope}/${props.scopeId}/${row.id}`}><FontIcon name="pencil" /></Link>;
    } else {
      return <Link to={`/report-templates/${props.scope}/${props.scopeId}/${row.id}`}><FontIcon name="search-plus" /></Link>;
    }
  }
  ...
  const nameFormatter = (cellContent, row, rowIndex) => {
    if (row.description) {
      ...
      return <OverlayTrigger trigger="hover" placement="top" overlay={popoverHover}>
        <Link to={`/report-templates/${props.scope}/${props.scopeId}/${row.id}`}>{cellContent}</Link>
      </OverlayTrigger>;
    } else {
      return <Link to={`/report-templates/${props.scope}/${props.scopeId}/${row.id}`}>{cellContent}</Link>;
    }
  }
