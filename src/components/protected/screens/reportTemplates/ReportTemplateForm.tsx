...
import * as ReportTemplateHelper from "./ReportTemplateHelper";
import { Rights, Scope } from "../../../../models/Common";
import ReportTemplateTabContainer from "./ReportTemplateTabContainer";
...
const ReportTemplateForm = (props: ReportTemplateFormProps) => {
  ...
  const [templateScopeId, setTemplateScopeId] = useState<number>(-1);
  ...
  async function loadForm() {
    ...
    if (props.id > 0) {
      ...
      if (template) {
        ...
        setOwner(owner);
        const templateScope = _.get(template, "scope.scope", "all");
        const templateScopeId = _.get(template, "scope.scopeId", -1);
        await loadAttributes(templateScope, templateScopeId);
        await loadPresets(template.writeAccess, templateScope, templateScopeId);
        setTemplateScope(templateScope);
        setTemplateScopeId(templateScopeId);
      ...
      setTabs([]);
      setTabsValidation([]);
      await loadScopeFields(true, props.scope);
    }
  }
  ...
  async function loadScopeFields(writeAccess: boolean, level: ScopeType) {
    setShowEntityLoader(true);
    const entityOptions = await Helper.getEntityOptions("report-templates", props.scope, props.scopeId, level);
    const entityId = (entityOptions.length > 0 ? entityOptions[0].value : -1) as number;
    const scope = Helper.getScopeByLevel(level);
    await loadAttributes(scope, entityId);
    await loadPresets(writeAccess, scope, entityId);
    setLevel(level);
  ...
  const handleLevelChange = async (selected) => {
    await loadScopeFields(writeAccess, selected.value as ScopeType);
  }
  ...
  const saveClick = () => {
    ...
    if (nameValidation.error || invalidTabs) {
      ...
      if (props.id > 0) {
        ...
        if (level !== "root") {
          template.scope.scopeId = entityId;
        }
        props.onSubmit(props.id, template);
      }
    }
          ...
          <div className="row no-gutters">
            {props.id === -1 &&
              <div className="col-lg-6 no-gutters">
                <Form.Group controlId="template-scope">
                  <Form.Label>Level:</Form.Label>
                  <Select
                    inputId="react-select-template-scope"
            ...
            {level !== "root" && props.id === -1 &&
              <div className="col-lg-6 px-1">
                <Loader visible={showEntityLoader} loaderClass="loading-input" />
                {!showEntityLoader &&
                  <Form.Group controlId="template-entity">
                    <Form.Label>{Helper.getLabelByScopeType(level)}</Form.Label>
                    <Select
                      inputId="react-select-template-entity"
                      className="react-select-container"
                      classNamePrefix="react-select"
            ...
            {props.id > 0 && <Fragment>
              <div className="col-lg-6 px-1">
                <Form.Group controlId="template-level">
                  <Form.Label>Level:</Form.Label>
                  <Form.Control
                    disabled={true}
                    type="text"
                    value={Helper.getLabelByScope(templateScope)}
                  />
                </Form.Group>
              </div>
              <div className="col-lg-6 px-1">
                <Form.Group controlId="template-owner">
