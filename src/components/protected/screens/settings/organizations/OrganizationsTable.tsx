...
export default class OrganizationsTable extends Component<OrganizationsTableProps, OrganizationsTableState> {
  private tableId: string = "organizations-table";
  ...
  metrics(): Metric[] {
    if (this.props.videoMode) {
      if (this.props.rights.VIEW_FINANCIALS) {
        return [
          { col: 'displayName', label: 'name', type: 'organization', align: 'left' },
  ...
  editFormatter = (cellContent, row) => {
    if (row.writeAccess) {
      return <a onClick={(e) => { e.preventDefault(); this.props.editClick(row.id) }} href="" className="table-btn"><FontIcon name="pencil" /></a>
    } else {
      return "";
    }
  }
  ...
  getRows() {
    ...
    return this.props.organizations.map((o) => {
      const row = _.pick(o, metricValues);
      const id = _.get(o, "organization.id", -1);
      const writeAccess = o.rights.MANAGE_ORGANIZATION;
            ...
            <BootstrapTable
              ...
              pagination={paginationFactory({
                ...
                sizePerPageList: [{
                  ...
                  text: 'All', value: this.props.organizations.length
                }],
