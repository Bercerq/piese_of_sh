import React, { Component } from "react";
import BootstrapTable from "react-bootstrap-table-next";
import ToolkitProvider, { Search } from "react-bootstrap-table2-toolkit";
import paginationFactory from "react-bootstrap-table2-paginator";
import { Tooltip, OverlayTrigger } from "react-bootstrap";
import FontIcon from "../../../../UI/FontIcon";
import * as _ from "lodash";
import * as TableHelper from "../../../../../client/TableHelper";
import { Metric } from "../../../../../client/schemas";
import { Rights } from "../../../../../models/Common";
import { AppUser } from "../../../../../models/AppUser";


interface CampaignGroupsTableProps {
  records: any[];
  metrics: Metric[];
  user: AppUser;
  rights: Rights;
  type: "recent" | "older" | "archived";
  editClick?: (id: string | number, writeAccess: boolean) => void;
  archiveClick?: (id: string | number) => void;
  restoreClick?: (id: string | number) => void;
}

interface CampaignGroupsTableState {
  page: number;
  sizePerPage: number | "all";
  defaultSorted: { dataField: string; order: string; };
}

export default class CampaignGroupsTable extends Component<CampaignGroupsTableProps, CampaignGroupsTableState> {
  private tableIdprefix: string = "campaigngroups-table-";

  constructor(props, context) {
    super(props, context);
    this.state = {
      page: 1,
      sizePerPage: TableHelper.getInitialSizePerPage(this.tableIdprefix + this.props.type, this.props.user),
      defaultSorted: this.defaultSorted()
    };
  }

  defaultSorted(): { dataField: string; order: string; } {
    if (this.props.type === "recent") {
      return { dataField: "endDate", order: "asc" };
    } else {
      return { dataField: "endDate", order: "desc" };
    }
  }

  onSort = (dataField, order) => {
    this.setState({ defaultSorted: { dataField, order } });
  }

  onSizePerPageChange = (sizePerPage, page) => {
    TableHelper.storeSizePerPage(sizePerPage, this.tableIdprefix + this.props.type, this.props.user);
    this.setState({ sizePerPage });
  }

  getColumns() {
    const hiddenCols: any[] = [{ dataField: 'writeAccess', text: '#', hidden: true }];
    const metricCols: any[] = this.props.metrics.map((o) => {
      return {
        dataField: o.col,
        text: o.label || o.col,
        align: o.align || "left",
        headerAlign: o.align || "left",
        sort: true,
        onSort: this.onSort,
        sortCaret: TableHelper.columnSort,
        formatter: TableHelper.getFormatter(o.type)
      }
    });

    const editCols = this.getEditCols();
    return hiddenCols.concat(editCols, metricCols);
  }

  getEditCols() {
    if (this.props.type === "archived") {
      return [{
        dataField: 'df3',
        isDummyField: true,
        text: '',
        formatter: this.restoreFormatter,
        headerStyle: { width: "30px" }
      }
      ]
    } else {
      return [{
        dataField: 'df1',
        isDummyField: true,
        text: '',
        formatter: this.editFormatter,
        headerStyle: { width: "30px" }
      },
      {
        dataField: 'df2',
        isDummyField: true,
        text: '',
        formatter: this.archiveFormatter,
        headerStyle: { width: "30px" }
      }]
    }
  }

  editFormatter = (cellContent, row) => {
    if (row.writeAccess) {
      return <a onClick={(e) => { e.preventDefault(); this.props.editClick(row.id, row.writeAccess) }} href="" className="table-btn"><FontIcon name="pencil" /></a>;
    }
    return null;
  }

  archiveFormatter = (cellContent, row) => {
    if (row.writeAccess) {
      const rowTooltip = <Tooltip id={"archive-tooltip-" + row.id}>{"archive"}</Tooltip>;
      return <OverlayTrigger placement="top" overlay={rowTooltip}>
        <a onClick={(e) => { e.preventDefault(); this.props.archiveClick(row.id) }} href="" className="table-btn"><FontIcon name="archive" /></a>
      </OverlayTrigger>;
    }
    return null;
  }

  restoreFormatter = (cellContent, row) => {
    if (row.writeAccess) {
      const rowTooltip = <Tooltip id={"restore-tooltip-" + row.id}>{"restore"}</Tooltip>;
      return <OverlayTrigger placement="top" overlay={rowTooltip}>
        <a onClick={(e) => { e.preventDefault(); this.props.restoreClick(row.id) }} href="" className="table-btn"><FontIcon name="repeat" /></a>
      </OverlayTrigger>;
    }
    return null;
  }

  getRows() {
    const metricValues = this.props.metrics.map((o) => { return o.col; });
    return this.props.records.map((o) => {
      const row = _.pick(o, metricValues);
      const id = _.get(o, "id", -1);
      const writeAccess = o.rights.MANAGE_CAMPAIGNGROUP;
      return _.assign({ id, writeAccess }, row);
    });
  }

  getCurrentPage() {
    const lastPage = TableHelper.getLastPage(this.getRows(), this.state.sizePerPage);
    if (this.state.page > lastPage) {
      return lastPage;
    } else {
      return this.state.page;
    }
  }

  render() {
    const { SearchBar } = Search;
    const rows = this.getRows();
    const columns = this.getColumns();
    const sizePerPage = TableHelper.getSizePerPage(this.state.sizePerPage, rows.length);

    return <ToolkitProvider
      keyField="id"
      data={rows}
      columns={columns}
      search
    >
      {
        props => (
          <div>
            <div className="search-label-container">
              <FontIcon name="search" />
              <SearchBar
                {...props.searchProps}
                placeholder="Filter records"
                tableId={this.tableIdprefix + this.props.type}
              />
            </div>
            <BootstrapTable
              {...props.baseProps}
              id={this.tableIdprefix + this.props.type}
              classes="campaigngroups-table table-borderless"
              bordered={false}
              hover={true}
              striped
              keyField="id"
              data={rows}
              columns={columns}
              defaultSorted={[this.state.defaultSorted]}
              pagination={paginationFactory({
                page: this.getCurrentPage(),
                sizePerPage,
                showTotal: true,
                prePageText: "Previous",
                nextPageText: "Next",
                sizePerPageList: [{
                  text: '30', value: 30
                }, {
                  text: '50', value: 50
                }, {
                  text: '100', value: 100
                }, {
                  text: 'All', value: this.props.records.length
                }],
                onPageChange: (page, sizePerPage) => {
                  this.setState({ page });
                },
                onSizePerPageChange: this.onSizePerPageChange,
              })}
            />
          </div>
        )
      }
    </ToolkitProvider>;
  }
}